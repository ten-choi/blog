...
### Redo 로깅과 DML 성능
#### Redo 로그
오라클은 데이터 파일과 컨트롤 파일에서 발생하는 모든 변경사항을 Redo 로그에 기록
...
변경된 메모리 버퍼 블록과 데이터 파일 블록 간 동기화는 DBWR(DB Writer)·Checkpoint 프로세스를 이용해 추후 배치 방식으로 일괄 수행

*** 변경된 메모리 버퍼 블록을 디스크 상의 데이터 블록에 반영하는 작업은 **랜덤 액세스**라 매우 느림 → 그래서 Redo 로그는 **Append(순차 쓰기)** 방식을 쓰는 것
