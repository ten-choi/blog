---
title: "2.7 소켓 프로그래밍: 네트워크 애플리케이션 생성"
labels: [네트워크, 애플리케이션 계층, 소켓 프로그래밍, TCP 소켓, UDP 소켓]
published: 
date: 
...
이 패킷이 송신자의 소켓을 통과하면, 인터넷은 목적지 주소를 이용해 그 패킷을 수신 프로세스의 소켓으로 라우팅한다.

패킷이 수신 소켓에 도착하면 수신 프로세스는 소켓을 통해 그 패킷을 추출하고, 패킷의 내용을 조사하여 적절한 동작을 취한다.
...
즉, 패킷에는 IP 주소와 포트 번호로 구성된 목적지 주소가 붙게 되며, 송신자의 출발지 주소(IP와 port)도 붙게 된다.
...
소켓을 생성한 후 클라이언트는 3-way handshake를 거쳐 서버와 TCP 연결을 설정한다. (핸드셰이킹은 프로그램에서 전혀 인지하지 못한다.)
